...
const PayButton = (props: Props) => {
  ...
  const title =
    props.title ??
    `${props.caption ?? ""} ${props.currency?.toUpperCase() ?? "ETH"} Pay`;
  ...
  const send = async () => {
    ...
    if (props.currency === "eth") {
      ...
      hash = await Metamask.sendTransaction({
        ...
        gas: "0x186A0",
        value: "0x00",
        data: Payment.buildErc20Transfer(
          props.to,
          ethers.utils
            .parseUnits(`${props.value ?? 0}`, contract.decimals)
            .toHexString()
        ),
      });
    }
  ...
  const jumpToPay = () => {
    window.open(
      `https://ethpay.now.sh/to/${props.to}?value=${
        props.value ?? 0
  ...
  useEffect(() => {
    setHasWeb3(Metamask.hasWeb3());
  }, []);

  return (
    <button
      className={`${props.className ?? ""} __ethpay_button ${props.mode ?? ""}`}
